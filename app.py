...
def load_models():
    # DiT 문서 분류
    dit_processor = AutoProcessor.from_pretrained("microsoft/dit-base-finetuned-rvlcdip", local_files_only=True)
    dit_model = AutoModelForImageClassification.from_pretrained("microsoft/dit-base-finetuned-rvlcdip", local_files_only=True)
    
    # OCR
    ocr = PaddleOCR(lang='korean')
    
    # Donut (영수증 전용)
    donut_processor = AutoProcessor.from_pretrained("naver-clova-ix/donut-base-finetuned-cord-v2", local_files_only=True)
    donut_model = VisionEncoderDecoderModel.from_pretrained("naver-clova-ix/donut-base-finetuned-cord-v2", local_files_only=True)
    
    # LayoutLMv3
    layout_processor = LayoutLMv3Processor.from_pretrained("microsoft/layoutlmv3-base", apply_ocr=False, local_files_only=True)
    layout_model = LayoutLMv3ForTokenClassification.from_pretrained("microsoft/layoutlmv3-base", local_files_only=True)
    
    # 텍스트 요약
    # 원래 지정되어 있던 gangyeolkim/kobart-korean-summarizer-v2 저장소가 접근 불가(비공개/삭제)로
    # 전환되어, 공개된 KoBART 요약 모델로 대체
    summarizer_tokenizer = AutoTokenizer.from_pretrained("gogamza/kobart-summarization", local_files_only=True)
    summarizer_model = AutoModelForSeq2SeqLM.from_pretrained("gogamza/kobart-summarization", local_files_only=True)
    
    # 임베딩 모델 (벡터 검색용)
    embedding_model = SentenceTransformer("jhgan/ko-sroberta-multitask", local_files_only=True)
...
def extract_text_with_layout(image, ocr):
    # load_models()에서 캐시된 PaddleOCR 인스턴스를 재사용 (매번 새로 만들면 호출마다 수 초씩 소요됨)
...
# 벡터 유사도 검색
def search_by_similarity(query, embedding_model, session, top_k=10, min_score=0.2):
    # 쿼리 임베딩 생성
    ...
    filtered = [doc for doc, sim in top_matches if sim >= min_score]

    # OCR output can be noisy, so keep semantic search useful instead of showing an empty result page.
    if not filtered and top_matches:
        return [doc for doc, sim in top_matches[:3]]
